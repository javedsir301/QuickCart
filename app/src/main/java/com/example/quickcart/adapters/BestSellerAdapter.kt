...
import com.example.quickcart.databinding.ItemViewBestsellerBinding
import com.example.quickcart.models.BestSeller

class BestSellerAdapter(val seeAllBtnClicked: (BestSeller) -> Unit) :
    RecyclerView.Adapter<BestSellerAdapter.BestSellerAdapterViewHolder>() {
    ...
    override fun onBindViewHolder(holder: BestSellerAdapterViewHolder, position: Int) {
        ...
        holder.binding.apply {
            ...
            for (i in 0 until minimumSize) {
                ...
                Glide.with(holder.itemView).load(productType.products[i].productImagesUris?.get(0))
                    .into(listOfIv[i])
            }
